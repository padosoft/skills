...
_38 skills across 9 profiles, grouped by what you install. A skill that belongs to two profiles appears in both. The complete list with descriptions is [at the end](#every-skill-one-by-one); `make catalog` refreshes both._

### `core` — 16 skills · installed **globally**, they apply to every project whatever the stack
...
### `laravel` — 7 skills · Laravel and PHP applications
...
### `node` — 5 skills · Node and Bun services
...
### `react-native` — 4 skills · React Native and Expo apps
...
_All 38 skills in alphabetical order: what each one says, and when it triggers. Generated from the frontmatter._
...
`padosoft-durable-effects` · profiles: `api`, `data` · scope: `project` · version: 0.1.0
...
`padosoft-evidence-boundaries` · profiles: `core` · scope: `global` · version: 0.2.0
...
`padosoft-failure-visibility` · profiles: `core` · scope: `global` · version: 0.2.0
...
`padosoft-payments-reconciliation` · profiles: `payments` · scope: `project` · version: 0.1.0
