...
_40 skills across 9 profiles, grouped by what you install. A skill that belongs to two profiles appears in both. The complete list with descriptions is [at the end](#every-skill-one-by-one); `make catalog` refreshes both._
...
### `api` — 6 skills · HTTP APIs, their contracts, and the work they hand off
...
### `data` — 3 skills · the database, and everything read from it in volume
...
| **Project** | The stack of that repository (`api`, `data`, `devops`, `email`, `laravel`, `node`, `payments`, `react-native`) | Skills travel with the project, not with the machine |
...
_All 40 skills in alphabetical order: what each one says, and when it triggers. Generated from the frontmatter._
