...
invariants**, **docs match code**, **evidence boundaries**, **tenant isolation**, **contract changes**,
**verify before writing** and **environment gating** — rules that stacks sharing no code arrived at
independently, which is what earns a place there. If you do not know which profile you need, install `core` and ask the agent.
...
_33 skills across 9 profiles, grouped by what you install. A skill that belongs to two profiles appears in both. The complete list with descriptions is [at the end](#every-skill-one-by-one); `make catalog` refreshes both._

### `core` — 13 skills · installed **globally**, they apply to every project whatever the stack
...
_All 33 skills in alphabetical order: what each one says, and when it triggers. Generated from the frontmatter._
...
`padosoft-ci-workflow-gates` · profiles: `devops` · scope: `project` · version: 0.2.0
...
`padosoft-logging-discipline` · profiles: `core` · scope: `global` · version: 0.2.0
