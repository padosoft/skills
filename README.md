...
invariants**, **docs match code**, **evidence boundaries** and **tenant isolation** — rules that stacks
sharing no code arrived at independently, which is what earns a place there. If you do not know which profile you need, install `core` and ask the agent.
...
_27 skills in the catalog. Section generated from the frontmatter: refresh it with `make catalog`._
...
| [`padosoft-evidence-boundaries`](#evidence-boundaries) | core | global | 0.1.0 |
...
**`padosoft-evidence-boundaries`** · profiles: `core` · scope: `global` · version: 0.1.0
