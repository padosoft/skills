...
_35 skills across 9 profiles, grouped by what you install. A skill that belongs to two profiles appears in both. The complete list with descriptions is [at the end](#every-skill-one-by-one); `make catalog` refreshes both._
...
### `node` — 3 skills · Node and Bun services
...
### `api` — 5 skills · HTTP APIs, their contracts, and the work they hand off
...
_All 35 skills in alphabetical order: what each one says, and when it triggers. Generated from the frontmatter._
...
`padosoft-api-security-review` · profiles: `api`, `node` · scope: `project` · version: 0.2.0
